...
// With minor modifications (mostly conversion from C-style to C++-style).
...
RGBd xyz_to_rgb(const ColourSystem &cs, const XYZd &xyz) {
  colreal_t xr, yr, zr, xg, yg, zg, xb, yb, zb;
  colreal_t xw, yw, zw;
  colreal_t rx, ry, rz, gx, gy, gz, bx, by, bz;
  colreal_t rw, gw, bw;
  ...
  return RGBd{
      colreal_t((rx * xyz[0]) + (ry * xyz[1]) + (rz * xyz[2])),
      colreal_t((gx * xyz[0]) + (gy * xyz[1]) + (gz * xyz[2])),
      colreal_t((bx * xyz[0]) + (by * xyz[1]) + (bz * xyz[2]))
  };
}
